...
def gram_matrix(features, normalize=True):
    ...
    M = H * W
    f = torch.reshape(features, (N, C, M))
    ft = torch.transpose(f, 1, 2)
    G = torch.bmm(f.view(N, C, M), ft.view(N, M, C))

    return G / (C * M) if normalize else G
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, L in enumerate(style_layers):
        ...
        l = content_loss(style_weights[i], G1, style_targets[i])
        # loss1 = style_weights[i] * ((G1 - style_targets[i]) ** 2).sum()
        loss += l
...
def tv_loss(img, tv_weight):
    ...
    horizontal = torch.sum(torch.pow(img[:, :, :, 1:] - img[:, :, :, :-1], 2))
    vertical = torch.sum(torch.pow(img[:, :, 1:] - img[:, :, :-1], 2))
