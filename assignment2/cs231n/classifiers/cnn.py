...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params["W2"] = np.random.normal(scale=weight_scale, size=(int(num_filters * input_dim[1] * input_dim[2] / 4), hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        grads["b1"] = db1 + 2 * reg * self.params["b1"]
        grads["W2"] = dw2 + 2 * reg * self.params["W2"]
        grads["b2"] = db2 + 2 * reg * self.params["b2"]
        grads["W3"] = dw3 + 2 * reg * self.params["W3"]
        grads["b3"] = db3 + 2 * reg * self.params["b3"]
